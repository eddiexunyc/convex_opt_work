...
The first function appears to be a quasiconvex because the sublevel sets appear to be convex. It is definitely not concave or quasiconcave because the superlevel sets are not convex.
...
# define the objective function f(x, y) = 1 / (x * y)
f = cp.inv_pos(x * y)
...
f = cp.sqrt(1 + cp.quad_over_lin(x, y))
...
For this problem, we will be working with the dataset [nyc_crashes.csv](https://github.com/georgehagstrom/DATA609Spring2025/blob/main/website/assignments/labs/labData/nyc_crashes.csv), which contains a time series of the number of car crashes occurring in Manhattan every hour for a period of time.
