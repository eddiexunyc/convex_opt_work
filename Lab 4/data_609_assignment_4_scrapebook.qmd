...
(b) The log-likelihood function $L$ can be minimized to find the maximum likelihood estimate for the $\lambda$ coefficients. Formulate this constrained optimization problem in `CVX` and solve it for the NYC crashes dataset, that is solve:
...
crash_data_revised = crash_data.drop(columns = ['date', 'day'], axis=1)

# define variables
C = crash_data_revised.values
N, T = C.shape

# define lambda
lambda_ = cp.Variable(T, pos=True) 
L = N * cp.sum(lambda_) - cp.sum(C * cp.log(lambda_))

# formulate the problem: minimize the negative log-likelihood
problem = cp.Problem(cp.Minimize(L))

# solve the problem
problem.solve()

# Plot the estimated λ values
hours = 24
plt.plot(hours, lambda_.value, marker='o')
plt.title('Estimated Crash Rates (λ) for Each Hour of the Day')
plt.xlabel('Hour of the Day')
plt.ylabel('Estimated λ (Crash Rate)')
plt.xticks(hours)
plt.show()
```
