...
# define the objective for the least-squares error for uncensored data and the error for censored data (which is constrained)
objective = cp.Minimize(cp.sum_squares(y_uncensored - X_uncensored @ c) + cp.sum_squares(y_censored - X_censored @ c))
...
# load train data
train_url = 'https://media.githubusercontent.com/media/georgehagstrom/DATA609Spring2025/refs/heads/main/website/assignments/labs/labData/rob_regression.csv'
train_data = pd.read_csv(train_url, index_col='Unnamed: 0')

# load test data
test_url = 'https://media.githubusercontent.com/media/georgehagstrom/DATA609Spring2025/refs/heads/main/website/assignments/labs/labData/rob_regression_test.csv'
...
# (b) For robust logistic regression

# Define epsilon (the perturbation magnitude)
epsilon = 0.1  # You can adjust this value based on your data

# Get the number of features
d = X_train.shape[1]

# Define the theta variable (model parameter)
theta = cp.Variable(d)

# Initialize the robust logistic loss
loss = 0

# Loop over each training sample to add the robust logistic loss term
for i in range(X_train.shape[0]):
    xi = X_train[i]  # The feature vector (now using numpy array indexing)
    yi = y_train[i]  # The label (now using numpy array indexing)
    
    # The logistic loss term with the worst-case perturbation
    log_term = cp.log(1 + cp.exp(-yi * (xi @ theta)))  # Standard logistic loss term
    perturbation = epsilon * cp.norm1(theta)  # The perturbation term
    
    # Add the perturbation to the logistic loss term
    loss += log_term + perturbation  # Sum up the loss for each example

# Objective is to minimize the robust logistic loss
...
# Get the optimized theta from the cvxpy solver
theta_robust = theta.value

# Predict on the test set using the robust model
y_pred_robust = np.sign(X_test.dot(theta_robust))

# Calculate accuracy (test error rate) for the robust model
accuracy_robust = accuracy_score(y_test, y_pred_robust)
print(f"Robust Logistic Regression Test Accuracy: {accuracy_robust}")
```
