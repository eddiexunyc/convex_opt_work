...
where:  $a_1 < a_2 < \cdots < a_n$. Of course $\mathbf{p} \in \mathbb{R}^n$ lies in the standard probability simplex $P \{\mathbf{p}\, |\, 1^T \mathbf{p} = 1, \mathbf{p} \preceq 0\}$. 
...
c. The set of all $\mathbf{p}$ such that the expectation of $|x|^3$ is greater than a given constant $\alpha$ times the expectation of $|x|$: $E|x^3| $\leq$ $\α E|x|$
...
c. The set of all $\mathbf{p}$ such that the expectation of $|x|^3$ is greater than a given constant $\alpha$ times the expectation of $|x|$: $E|x^3| $\leq$ $\α E|x|$ can be defined as a convex set because the constraint is equivalent to a linear inequality in the probabilities $p_i$.
