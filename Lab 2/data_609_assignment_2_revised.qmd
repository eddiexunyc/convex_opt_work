...
for t in range(days, temp_length):
    a_t = np.concatenate([trips[t-days:t][::1], [temp[t]]])  
    b_t = trips[t]

    if t == days:
        x = np.column_stack([trips[days-7:days][::-1],temp[days]])
        y = trips[days]
        ...
        x_t = np.column_stack(trips[t-days:days][::-1],temp[t])
        y_t = trips[days]
        x_ols_t = sm.add_constant(x_t)
        model_t = sm.OLS(y_t, x_ols_t)
        result_t = model_t.fit()
        coefficients.append(result_t.params)
        r2_scores.append(result_t.rsquared)
...
for i in range(coefficients.shape[1]): 
    plt.plot(range(days, temp_length), coefficients[:, i], label=f'Coefficient {i+1}')
plt.xlabel('Time (t)')
...
plt.figure(figsize=(10, 6))
plt.plot(range(days, temp_length), r2_scores, label='R-squared', color='r')
plt.xlabel('Time (t)')
...
for t in range(days, len(trips)):
    ...
    # solve for r_squared
    predictions = np.dot(a_t, coefficients[t-days])
...
# plot (NEED TO FIX)
plt.figure(figsize=(12, 6))
plt.plot(coeff_nonlinear[:, :-1]) 
plt.title('Coefficients (Nonlinear Model [First 7 days]) over Time')
plt.xlabel('Time')
plt.ylabel('Coefficient Value')

# plot coefficient
plt.plot(coeff_nonlinear[:, -1], label='(Nonlinear)')  
plt.title('Coefficient (Nonlinear) over Time')
plt.xlabel('Time')
plt.ylabel('Coefficient Value')
plt.tight_layout()
plt.show()
...
for r in r_targets:
    ...
    constraints = [cp.sum(w) == 1, w.T @ mu == r]
    ...
    portfolio_returns = (train_data @ optimal_weights).cumsum()

    # calculate the portfolio return 
    # portfolio_cum_returns_train[r] = np.exp((train_data @ optimal_weights[int(r)]).cumsum())
