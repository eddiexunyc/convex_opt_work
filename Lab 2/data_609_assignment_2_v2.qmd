...
(b) We have included temperature as a variable because it probably influences the decision to ride a citibike. However, the relationship could be nonlinear, as both extreme high and low temperatures make bike riding less comfortable. With this idea in mind, create a new feature by choosing a nonlinear function of the temperature $T$ that represents
the potential for both high and low values of $T$ to the same impact on predicted ridership. 
...
citi_data <- read_csv(citi_bike_raw_link)

# define variables
days <- 7
trip_length <- nrow(citi_data)
G <- matrix(0, nrow = days + 1, ncol = days + 1) # gram matrix
h <- matrix(0, nrow = days + 1, ncol = 1) # initial vector
coefficient_array <- matrix(0, nrow = trip_length - days, ncol = days + 1) 

# perform the RLS
for (t in (days + 1):trip_length) {
  a_t <- c(citi_data$daily_trips[t-1:t-days], citi_data$TMAX[t])
  b_t <- citi_data$daily_trips[t]

  # update gram matrix and vector
  G <- t(a_t) %*% a_t
  h <- h + b_t * a_t
  
  # solve for coefficient and update it
  x_t <- solve(G, h)
  coefficient_array[t - days, ] <- x_t
}

# get temperature coefficients
temperature_coefficients <- coefficient_array[, days + 1]

# plot
ggplot(data = data.frame(time = (days + 1):n, C_T = temperature_coefficients), aes(x = time, y = C_T)) +
  geom_line() +
  labs(title = "Change in Temperature Coefficient over Time",
       x = "Time", y = "Temperature Coefficient") +
  theme_minimal()

# calculate r squared
r_squared_value <- numeric(n - days)

for (t in (days + 1):trip_length) {
  a_t <- (citi_data$daily_trips[t-1:t-7], citi_data$high_temperature[t])
  prediction <- sum(coefficient_array[t - days, ] * a_t)
  actual <- citi_data$daily_trips[t]
  residual <- actual - prediction
  tss <- sum((citi_data$daily_trips[(days + 1):t] - mean(citi_data$daily_trips[(days + 1):t]))^2)
  rss <- sum(residual^2)
  r_squared_value[t - days] <- 1 - rss / tss
}

# plot
ggplot(data = data.frame(time = (days + 1):n, R_squared = r_squared_value), aes(x = time, y = R_squared)) +
  geom_line() +
  labs(title = "Change in R-squared over Time",
       x = "Time", y = "R-squared") +
  theme_minimal()
```
...
# read in data
social_mob_link = 'https://media.githubusercontent.com/media/georgehagstrom/DATA609Spring2025/refs/heads/main/website/assignments/labs/labData/social_mobility.csv'
social_mobility_data = pd.read_csv(social_mob_link)
...
# read in data
stock_return_link = 'https://media.githubusercontent.com/media/georgehagstrom/DATA609Spring2025/refs/heads/main/website/assignments/labs/labData/stock_returns.csv'
stock_return_data = pd.read_csv(stock_return_link)
