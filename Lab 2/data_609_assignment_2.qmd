...
data = pd.read_csv(citi_bike_raw_link)


# Prepare the data for autoregressive modeling
n = len(data)
lags = 7  # We are using the past 7 days

# Arrays to hold the coefficients and R^2 values
coefficients = np.zeros((n-lags, lags + 1))  # Coefficients for each time point
r_squared = np.zeros(n-lags)  # R^2 values for each time point

# Initialize Gram matrix and vector
G = np.eye(lags + 1)  # Initial Gram matrix (identity matrix)
h = np.zeros(lags + 1)  # Initial vector

# Recursive Least Squares calculation
for t in range(lags, n):
    # Build the design matrix A_t and target vector b_t for time t
    A_t = np.zeros(lags + 1)
    for i in range(lags):
        A_t[i] = data['daily_trips'][t-i-1]  # Past trip data
    A_t[lags] = data['TMAX'][t]  # Temperature data
    
    b_t = data['daily_trips'][t]  # The observed Citibike trip for the current day
    
    # Update the Gram matrix and vector
    G = G + np.outer(A_t, A_t)
    h = h + b_t * A_t
    
    # Solve for the coefficients
    x_t = np.linalg.solve(G, h)
    coefficients[t-lags] = x_t  # Store the coefficients
    
    # Calculate the predicted value and R^2
    prediction = np.dot(A_t, x_t)
    residual = b_t - prediction
    ss_total = np.sum((data['daily_trips'][lags:] - np.mean(data['daily_trips'][lags:]))**2)
    ss_residual = np.sum(residual**2)
    r_squared[t-lags] = 1 - (ss_residual / ss_total)

plt.figure(figsize=(10, 6))
for i in range(lags):
    plt.plot(range(lags, n), coefficients[:, i], label=f'$C_{i+1}$')
plt.plot(range(lags, n), coefficients[:, -1], label='$C_T$', linestyle='dashed', color='black')
...
# Plot R^2 over time
plt.figure(figsize=(10, 6))
plt.plot(range(lags, n), r_squared, label='R^2')
plt.xlabel('Time (Days)')
plt.ylabel('R^2')
